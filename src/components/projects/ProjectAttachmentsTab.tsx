...
import { Upload, File, Download, Trash2, FileText, Image as ImageIcon, Paperclip, Loader2 } from "lucide-react";
...
interface Attachment {
  ...
  uploaded_by: string;
  uploaded_at: string;
  isUploading?: boolean; // Add flag for optimistic UI
}
...
export function ProjectAttachmentsTab({ project, currentUser }: ProjectAttachmentsTabProps) {
  ...
  const fetchAttachments = async () => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-c142e950/projects/${project.id}/attachments`,
        {
          headers: {
  ...
  const handleDelete = async (attachmentId: string) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-c142e950/projects/${project.id}/attachments/${attachmentId}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`,
